		...
		\subsubsection{Thermodynamic Potentials}
			...
			\begin{table}[ht]
				...
				\begin{tabular}{ r | l | l | l }
					...
					Entropy & $S(U,V)$ & $\diff S = \frac{1}{T} \diff U + \frac{p}{T} \diff V$ & $\left(\dpder{}{V} \dfrac{1}{T}\right)_U = \left(\dpder{}{U}\dfrac{p}{T}\right)_V$ \\ \hline \xrowht{26pt}
					Internal Energy & $U(S,V)$ & $\diff U = T \diff S - p \diff V$ & $\left( \dpder{T}{V} \right)_S =	- \left( \dpder{p}{S} \right)_V$ \\ \hline \xrowht{26pt}
					Free Energy & $F(V,T) = U - TS$ & $\diff F = -S \diff T - p \diff V$ & \phantom{-} $\left(\dpder{S}{V}\right)_T = \left(\dpder{p}{T}\right)_V$ \\ \hline \xrowht{26pt}
					Gibbs energy & $G(T,p) = U - TS + pV$ & $\diff G = -S \diff T - V \diff p$ & $-\left(\dpder{S}{p}\right)_T = \left(\dpder{V}{T}\right)_p$ \\ \hline \xrowht{26pt}
					Enthalpy & $H(p,S) = U + pV$ & $\diff H = T \diff S + V \diff p$ & \phantom{-} $\left(\dpder{V}{S}\right)_p = \left(\dpder{T}{p}\right)_S$ \\ \hline \xrowht{26pt}
					Grand potential & $\Omega(T,V,\mu) = U - T S - \mu N$ & $\diff \Omega = -S \diff T - p \diff V -N \diff \mu$ &	\\ \hline
					\end{tabular}}
				\caption{List of common potentials}
