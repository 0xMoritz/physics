...
\section{Quantum Field Theory\index{QFT}\index{Quantenfeldtheorie}} % QFt
	...
	\subsection{Fermions}
		...
		\label{Sec:FermionicCreationAndAnnihilationOperators}
			...
			\begin{equation}
				...
				\hspace{20pt}
				%\anticomm{\hat{a}_k}{\hat{a}_{k'}} = \anticomm{\hat{a}_k^\dagger}{\hat{a}_{k'}^\dagger} = 0
				\Big\lbrace\hat{a}_k,\hat{a}_{k'}\Big\rbrace = \anticomm{\hat{a}_k^\dagger}{\hat{a}_{k'}^\dagger} = 0
			\end{equation}
			In particular $\hat{a}_k^\dagger \hat{a}_k^\dagger = 0 = \hat{a}_k \hat{a}_k$, which ensures $n_k\in\cBr{0,1}$, i.e. the Pauli exclusion principle\index{Pauli!Ausschlussprinzip}.
	...
	\subsection{Second Quantization}
		...
		\noindent
		Fock space\index{Fock!Raum} (direct sum of Hilbert spaces\index{Hilbert!Raum} for all possible numbers of particles $N$; $\mathcal{H}^{(0)} = \cBr{\Ket{0}}$ consists only of the vacuum state):
		\begin{equation}
			\mathcal{F}
