...
\section{Quantum Field Theory\index{QFT}\index{Quantenfeldtheorie}} % QFT
	...
	\subsection{Basics}
		...
		\subsubsection{Gamma Matrices}
			...
			\noindent
			$\gamma^5$-Matrix (using the convention $\levicivita{0123} = +1$):
			\begin{equation}
				\gamma^5 := \i\gamma^0 \gamma^1 \gamma^2 \gamma^3
				= \frac{\i}{4!}\levicivita{\mu\nu\rho\sigma} \gamma^\mu \gamma^\nu \gamma^\rho \gamma^\sigma
			\end{equation}
	...
	\subsection{Fermions}
		...
		\subsubsection{Two-component Spinor Formalism}
			...
			\begin{equation}
				\varepsilon^{12} = - \varepsilon^{21} = \varepsilon_{21} = - \varepsilon_{12} = 1
			\end{equation}

			\noindent
			Dirac fermion\index{Dirac!Fermion} $\Psi$ is described by a pair of Graßmann-valued fields $\psi_{\alpha}, \bar{\psi}^c_{\dot{\alpha}}$.
			\begin{align}
	...
	\subsection{Standard Model of particle Physics\index{SM}}
		...
		\subsubsection{Electroweak Theory\index{EW Theory}}
			...
			\noindent
			Relation of coupling constants to the Weinberg angle:
			\begin{align}
				e = g \sin\theta_W = g' \cos\theta_W
				\hsp \sin \theta_W = \frac{g'}{\sqrt{g^2 + g'^2}}
