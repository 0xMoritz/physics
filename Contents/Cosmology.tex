...
\section{Cosmology\index{Kosmologie}}
	...
	\subsection{Big Bang Theory}
		...
		\begin{itemize}
			...
			\item \emph{Sachs--Wolfe effect\index{Sachs!--Wolfe Effekt}\index{Wolfe!Sachs--Wolfe Effekt}}: CMB photons at the surface of last scattering lose energy when climbing out of a potential well created by dark matter aggregates $\delta \hat{T} / \hat{T} = -\delta \hat{\phi} / 3c$, leading to a redshift.
	...
	\subsection{Structure Formation}
		...
		\noindent
		Jeans length / Jeans instability\index{Jeans!Kriterium} (Critical extend, above which a cloud of gas will gravitationally collapse; assumes $H=0$; mean molecular mass $\mu$):
		\begin{equation}
		...
		\noindent
		Press--Schechter mass function\index{Press!--Schechter Massenfunktion}\index{Schechter!Press--Schechter Massenfunktion} (where $\delta_c \approx 1.69$ is the linear density contrast where the non-linear halo collapses, $\sigma_R$ is the variance of the filtered density contrast field $\sigma_R^2 = 4\pi\int\frac{k^2 \dd k}{(2\pi)^3} P(k) \hat{W}_R^2(k)$ with some window function $\hat{W}(k)$, $D_+(a) = \delta/\delta_0$ is the growth factor of the linear density contrast):
		\begin{equation}
